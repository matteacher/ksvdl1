function I = displayDictionaryElementsAsImage(D, numRows, numCols,X,Y,sortVarFlag,pre)
...
%         end
            ...
            data = I(borderSize+(j-1)*sizeForEachImage+1:j*sizeForEachImage,borderSize+(i-1)*sizeForEachImage+1:i*sizeForEachImage,:);
            %imwrite(data,[path namepre '_' num2str(j) '_' num2str(i) '.bmp'])
            
            %imwrite(imresize(data,10,'nearest'),[pre  'Dic/' pre '_' num2str(counter) '.bmp'])
            %imwrite(data,[pre  'Dic/real/' pre '_' num2str(counter) '.bmp'])
        counter = counter+1;
    end
end
...
if (showImFlag) 
    I = I-min(min(min(I)));
    I = I./max(max(max(I)));
    %imwrite(imresize(I,4,'nearest'),[pre 'Dic/' pre 'Dic' '.bmp']);
    %imwrite(I,[pre 'Dic/real/' pre 'Dic' '.bmp']);
    imshow(I,[]);
end
