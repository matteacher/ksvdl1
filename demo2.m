...
%imwrite(uint8(IMin0),'IMin0.bmp')
IMin=IMin0+sigma*randn(size(IMin0)); %添加噪声作为输入图片
PSNRIn = 20*log10(255/sqrt(mean((IMin(:)-IMin0(:)).^2)));  %输入图片的噪声等级
%imwrite(uint8(IMin),'IMin.bmp')
